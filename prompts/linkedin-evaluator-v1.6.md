...
**Created:** 2026-03-01
**Updated:** 2026-03-02
**Purpose:** Deep LinkedIn profile analysis with recommendations verification, flexible education requirements, and approximate distance estimation to Fort Lee, NJ
...
You will be given a **LinkedIn profile URL**. Visit the profile and follow the steps below IN ORDER. Do not skip steps. Do not use your own judgment to override the rules.

---

## ⛔ STEP 0 — RUN THIS BEFORE ANYTHING ELSE: CONTRACTOR CHECK

Visit the LinkedIn profile. Scan for contractor indicators:
...
- Works through staffing agencies: Robert Half, TekSystems, Insight Global, Apex Systems, TEKsystems
- Profile headline says "available for contract" or "freelance developer"
- Has own LLC/company running alongside employment (e.g. "Owner at XYZ LLC" + another employer simultaneously)
...
```json
{
  ...
  "profile_url": "[their LinkedIn URL]",
...
⛔ **AUTO-DISQUALIFIERS — if ANY of these are true, set recommendation=NO_MATCH and pursue=false IMMEDIATELY. Do not override. Do not make exceptions.**
1. `is_contractor = true` → NO_MATCH, pursue=false (no exceptions — even if they are technically brilliant)
2. South Asian university → NO_MATCH, pursue=false
3. Non-US university + less than 12 years US work → NO_MATCH, pursue=false
4. `is_job_hopper = true` → NO_MATCH, pursue=false
5. No Flutter experience → NO_MATCH, pursue=false
6. No mobile development experience → NO_MATCH, pursue=false

**pursue field logic — this is MANDATORY:**
...
- pursue = false if is_contractor = true (regardless of recommendation)
...
After extracting the candidate's location, **estimate the approximate driving distance** to Fort Lee, NJ (40.8503° N, 73.9728° W) using your geographic knowledge.

**Reference distances from Fort Lee, NJ:**
- Fort Lee, NJ = 0 miles (on-site)
...
- Paramus, NJ = ~8 miles
- Westchester County, NY (general) = ~25-35 miles
...
- Connecticut (Stamford) = ~45 miles
- Connecticut (Hartford) = ~120 miles
- Boston, MA = ~250 miles

**Rules:**
- ≤ 50 miles = WITHIN RANGE ✅
- 51-75 miles = BORDERLINE (flag it, recruiter decides) ⚠️
- > 75 miles = OUT OF RANGE ❌
- Unknown location = flag as UNABLE_TO_ESTIMATE

Output fields:
- `estimated_miles_to_fort_lee`: number (your best estimate)
- `distance_confidence`: "HIGH" (major city/known location) or "LOW" (small town/suburb)
- `within_50_miles`: true/false/null
- `distance_notes`: brief explanation

**NOTE:** This is an approximation. Google Maps API integration is on the TODO list for precise calculations.
...
1. **STOP logic**: Contractor check now runs FIRST as Step 0. If contractor detected, outputs minimal JSON immediately and stops — no further evaluation. This prevents the AI from rationalizing contractors as strong matches.
2. **Early exit template**: Provides exact JSON to output for contractor disqualification
3. **pursue field**: Enforced as first field in all outputs

## Key Changes in v1.5

1. **Hard disqualifier enforcement**: Contractor = NO_MATCH is now absolutely non-negotiable. No exceptions.
2. **pursue field**: Now enforced as mandatory first field with explicit logic
3. **distance_estimation**: Now explicitly required in output

## Key Changes in v1.4

1. **Pursue flag**: Added top-level `pursue: true/false` field for instant recruiter filtering

## Key Changes in v1.3

1. **Distance estimation**: Claygent now estimates driving distance to Fort Lee, NJ using geographic knowledge (no API needed). Flags candidates >50 miles as out of range.
